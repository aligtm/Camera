...
//                    audioBuffer.put(audioData).position(0);
    ...
    public void startRecording(String filename,int width,int height,int rate) {
        startRecording(filename,width,height,rate, null);
    }

    public void startRecording(final String filename, final int width, final int height,int rate, final StartRecordingCallback recordingCallback) {
        queueEvent(new Runnable() {
            public void run() {
                ...
                } else if (CameraRecordGLSurfaceView.this.mFrameRecorder.startRecording(rate, filename,width,height)) {
